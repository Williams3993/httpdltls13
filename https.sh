#!/bin/bash
# TLS 1.3 Only HTTPS Proxy Setup Script
...
# 创建Caddy配置文件，强制使用TLS 1.3
echo "配置Caddy服务器，强制仅使用TLS 1.3，端口: $port..."
...
{
===========配置参数=============
代理模式：Https正向代理 (仅TLS 1.3)
地址：${domain}
端口：${port}
用户：${username}
密码：${password}
====================================
http=$domain:${port}, username=$username, password=$password, over-tls=true, tls-verification=true, tls-host=$domain, udp-relay=false, tls13=true, tag=https-tls13
}
EOF
...
echo ""
echo "=========客户端配置字符串=========="
echo "http=$domain:${port}, username=$username, password=$password, over-tls=true, tls-verification=true, tls-host=$domain, udp-relay=false, tls13=true, tag=https-tls13"
echo ""
